...
describe('numbers', function () {
  ...
  test('adaptive simpson should return an estimated definite integral of a function', function (done) {
    ...
    assert.equal(res - numbers.EPSILON < 666666.66667 < res + numbers.EPSILON, true);
    done();
  });
  ...
  test('limit should return the limit of a function at a given point from left, middle, or right', function (done) {
    ...
    assert.equal(res - numbers.EPSILON < 91.29 < res + numbers.EPSILON, true);
    done();
  });

  test('Stirling approximation gamma should return correct values', function (done) {
    assert.equal(numbers.EPSILON > 5.69718714897717 - calculus.StirlingGamma(0.1), true);
    assert.equal(numbers.EPSILON > 3.3259984240223925 - calculus.StirlingGamma(0.2), true);
    assert.equal(numbers.EPSILON > 2.3625300362696198 - calculus.StirlingGamma(0.3), true);
    assert.equal(numbers.EPSILON > 0.8426782594483921 - calculus.StirlingGamma(1.3), true);
    done();
  });

  test('Lanczos approximation gamma should return correct values', function (done) {
    assert.equal(numbers.EPSILON > 9.513507698668736 - calculus.LanczosGamma(0.1), true);
    assert.equal(numbers.EPSILON > 4.590843711998803 - calculus.LanczosGamma(0.2), true);
    assert.equal(numbers.EPSILON > 2.9915689876875904 - calculus.LanczosGamma(0.3), true);
    assert.equal(numbers.EPSILON > 0.8974706963062777 - calculus.LanczosGamma(1.3), true);
    done();
  });
